...
def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("VILLAIN_MUSIC/assets/wel2.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp, brightness_factor=brightness_factor) 
    pfp = pfp.resize((500, 500))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('VILLAIN_MUSIC/assets/font.ttf', size=60)
    welcome_font = ImageFont.truetype('VILLAIN_MUSIC/assets/font.ttf', size=60)
...
async def greet_new_member(_, member: ChatMemberUpdated):
    ...
    if member.new_chat_member and not member.old_chat_member and member.new_chat_member.status != "kicked":
        ...
        try:
            ...
            pic = "VILLAIN_MUSIC/assets/upic.png"
